...
import { Plus, X, ChevronLeft, Delete, CornerDownLeft, ArrowBigUp, ArrowRightToLine, Command, CaseSensitive, Camera, Home, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Menu, ChevronDown, ChevronRight, ChevronsUpDown, ChevronsDownUp } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col xl:flex-row flex-1 h-full gap-4 xl:gap-6 min-h-0">
      <div className="flex-[2] xl:flex-1 bg-neutral-900 border border-neutral-800 rounded-xl p-4 xl:p-8 flex flex-col items-center justify-center relative overflow-hidden min-h-0">
        ...
        {/* Full ANSI 104-Key Board Container */}
        <div className="flex gap-4 xl:gap-6 p-4 xl:p-6 bg-neutral-950 rounded-lg shadow-xl border border-neutral-800 z-10 overflow-x-auto w-full xl:w-max xl:max-w-full justify-start custom-scrollbar">
          ...
          <div className="grid grid-cols-3 gap-2 auto-rows-[3rem] h-max shrink-0 min-w-max">
            {NAV_KEYS.map((row, rIdx) => 
               row.map((keyId, cIdx) => renderKeyButton(keyId, 'w-12 h-12', {}))
            )}
          </div>

          {/* Numpad */}
          <div className="grid grid-cols-4 gap-2 auto-rows-[3rem] h-max shrink-0 min-w-max">
            {/* Row 0 Gap for F-Row spacing */}
          ...
          <div className="flex flex-col shrink-0 min-w-max ml-6 pt-[1rem]">
             <div className="flex flex-col gap-2 shrink-0 min-w-max bg-neutral-900 border border-neutral-800 rounded-[2.5rem] p-3 shadow-inner items-center relative">
               ...
               {/* Side buttons placed as physical floaters on the structure */}
      ...
      <div className="flex-1 xl:flex-none xl:w-96 bg-neutral-900 border border-neutral-800 rounded-xl flex flex-col overflow-hidden z-10 shrink-0 min-h-0">
        {currentAction ? (
          <div className="flex flex-col h-full min-h-0">
            <div className="p-4 border-b border-neutral-800 flex items-center gap-3 bg-neutral-950 shrink-0">
          ...
          <div className="flex flex-col h-full bg-neutral-950 min-h-0">
            <div className="p-4 border-b border-neutral-800 bg-neutral-900 shrink-0 flex items-center gap-3">
              <button onClick={() => setInspectedInputId(null)} className="p-1 hover:bg-neutral-800 rounded text-neutral-400 hover:text-white transition-colors">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <div>
                <h3 className="font-medium text-neutral-100">Inspecting Key</h3>
                <p className="text-xs text-neutral-500 font-mono">{renderKeyLabel(inspectedInputId)}</p>
              </div>
            </div>
                         ...
                         .map((combo, idx) => {
                           ...
                           return (
                             <div key={idx} className="flex gap-1 flex-wrap">
                               {combo.map(k => <span key={k} className={`px-1.5 py-0.5 bg-neutral-800 border ${k === inspectedInputId ? 'border-emerald-500/50 text-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.2)]' : 'border-neutral-700 text-neutral-400'} rounded text-[10px] font-mono leading-none flex items-center`}>{renderKeyLabel(k)}</span>)}
                             </div>
                           );
                         })}
                    </div>
                  ...
                  {!collapsedCategories.has(cat) && (
                    <div className="flex flex-col gap-1 mt-1 pl-2">
                      {groupedActions[cat].map(action => {
                        const isMapped = (activeSet.bindings.keyboard[action.id] || []).length > 0;
                        return (
                          <button
                            key={action.id}
                            onClick={() => {
